...
const monthlyPriceMigration = readFileSync(
  path.join(migrationsDirectory, '20260715154941_creator_monthly_price_24.sql'),
  'utf8',
)
const annualPriceMigration = readFileSync(
  path.join(migrationsDirectory, '202607290002_creator_annual_price_240.sql'),
  'utf8',
)
...
test('Creator $240 annual pricing is a new forward-only catalog revision', () => {
  const sql = normalize(annualPriceMigration)

  // Both earlier migrations are already deployed. Correcting the annual price
  // must add a revision, never rewrite the history those installs replayed.
  assert.match(
    initialCatalog,
  ...
  assert.match(
    monthlyPriceMigration,
    /monthly_price_currency_micros = 24000000::bigint/,
    'the already-deployed monthly-price migration must remain unchanged',
  )

  assert.match(sql, /^begin;/)
  assert.match(sql, /commit;$/)
  assert.match(sql, /in access exclusive mode;/)
  assert.match(sql, /billing-2026-07-29-creator-annual-240/)
  ...
  assert.match(update[1], /pricing_version = 'billing-2026-07-29-creator-annual-240'/)
  assert.match(update[1], /annual_price_currency_micros = 240000000::bigint/)
  assert.doesNotMatch(update[1], /monthly_price_currency_micros/)
...
test('Creator annual revision preserves paid-period immutability before updating', () => {
  const sql = normalize(annualPriceMigration)
  ...
  // Fail closed: run only from the $24-monthly revision, or re-run harmlessly
  // once already at the target. Any other catalog state aborts.
  assert.match(sql, /v_source_version constant text := 'billing-2026-07-15-creator-monthly-24'/)
  assert.match(sql, /v_target_version constant text := 'billing-2026-07-29-creator-annual-240'/)
  assert.match(sql, /v_creator\.pricing_version = v_source_version and v_creator\.annual_price_currency_micros = 300000000::bigint/)
  assert.match(sql, /v_creator\.pricing_version = v_target_version and v_creator\.annual_price_currency_micros = 240000000::bigint/)

  // Values this migration does not own are verified, not blessed.
  assert.match(sql, /v_creator\.monthly_price_currency_micros <> 24000000::bigint/)
