...
-- Forward-only Creator catalog revision. Installations that already applied
-- the original billing catalog keep every historical paid snapshot intact;
-- only a new Creator checkout may observe the revised monthly price.
--
...
declare
  ...
  v_target_version constant text := 'billing-2026-07-15-creator-monthly-24';
begin
  ...
  if not (
    (
      v_creator.pricing_version = 'billing-2026-07-15'
      and v_creator.monthly_price_currency_micros = 30000000::bigint
    )
    or
    (
      v_creator.pricing_version = v_target_version
      and v_creator.monthly_price_currency_micros = 24000000::bigint
    )
  ) then
    raise exception 'Creator catalog is not at the expected source or target revision';
  end if;

  -- These values are deliberately not part of this price change. Checking
  -- them prevents a superficially successful deployment from blessing drift.
  if v_creator.currency_code <> 'USD'
    or v_creator.annual_price_currency_micros <> 300000000::bigint
  ...
  then
    raise exception 'Creator annual price, grants or top-up economics do not match the approved catalog';
  end if;
...
begin
  ...
  set
    pricing_version = 'billing-2026-07-15-creator-monthly-24',
